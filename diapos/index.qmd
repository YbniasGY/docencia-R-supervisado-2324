...
$$Y = f(\mathbf{X}) + \varepsilon = f\left(X_1, \ldots, X_p \right) + \varepsilon$$
...
* $\varepsilon$ será el [**error o ruido**]{.hl-yellow}, que debería ser reducido a **algo aleatorio (irreducible)** (en estadística SIEMPRE nos vamos a equivocar, si no existiese estaríamos en análisis).
...
$$\hat{Y} = \hat{f}(\mathbf{X})  = \hat{f}\left(X_1, \ldots, X_p \right), \quad \hat{\varepsilon} = \hat{Y} - Y$$
* $\hat{Y}$ serán las [**estimaciones**]{.hl-yellow}

* $\hat{f}$ será el [**modelo estimado**]{.hl-yellow}

* $\hat{\varepsilon}$ los [**errores estimados**]{.hl-yellow}
...
En el caso concreto de la [**regresión lineal**]{.hl-yellow} tendremos:

$$Y = f(\mathbf{X}) + \varepsilon = \beta_0 + \beta_1 X_1 + \ldots + \beta_p X_p  + \varepsilon$$

$$\hat{Y} = \hat{f}(\mathbf{X}) = \hat{\beta_0} + \hat{\beta_1} X_1 + \ldots + \hat{\beta_p} X_p $$

* El modelo $f(\cdot)$ será una [**recta/polinomio lineal**]{.hl-yellow}

* En el caso de la regresión lineal [**univariante**]{.hl-yellow} tendremos por tanto $f(X) = \beta_0 + \beta_1X$ ($p = 1$)

* El objetivo será obtener la estimación de los $\hat{\beta}$ tal que minimicemos $\hat{\varepsilon} = Y - \left( \beta_0 + \beta_1 X_1 + \ldots + \beta_p X_p \right)$

---

desarrollo sesgo vs varianza (sin entrar en train y test aún)
