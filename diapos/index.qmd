...
El objetivo ha sido obtener la estimación de los $\widehat{\beta}$ tal que [**minimicemos la varianza residual o suma de errores al cuadrado**]{.hl-yellow} $SSR$

$$SSR \left(\boldsymbol{\beta} \right) =  \displaystyle \sum_{i=1}^{n} \widehat{\varepsilon}_{i}^{2} = \displaystyle \sum_{i=1}^{n} \left(Y_i - \beta_0 - \beta_1 X_{i1} - \ldots - \beta_p X_{ip} \right)^2 = \left( \mathbf{Y} - \mathbf{X} \boldsymbol{\beta} \right)^{T}\left( \mathbf{Y} - \mathbf{X} \boldsymbol{\beta} \right) $$
...
$$\widehat{\boldsymbol{\beta}} = \arg \min_{\beta \in \mathbb{R}^{p+1}} SSR \left(\boldsymbol{\beta} \right)$$
...
$$\widehat{\boldsymbol{\beta}} = \arg \min_{\beta \in \mathbb{R}^{p+1}} SSR \left(\boldsymbol{\beta} \right)$$
 ...
 $$\widehat{\boldsymbol{\beta}}_{\lambda} = \arg \min_{\beta \in \mathbb{R}^{p+1}} \left( SSR \left(\boldsymbol{\beta} \right) + \lambda \left\| \beta_{-1} \right\| \right), \quad \lambda \geq 0$$
...
$$\widehat{\boldsymbol{\beta}}_{\lambda, \alpha} = \arg \min_{\beta \in \mathbb{R}^{p+1}} \left( SSR \left(\boldsymbol{\beta} \right) + \lambda \left(\alpha \left\| \beta_{-1} \right\|_1  + (1-\alpha) \left\| \beta_{-1} \right\|_{2}^{2} \right)\right), \quad \lambda \geq 0,~0 \leq \alpha \leq 1$$
...
$$\widehat{\boldsymbol{\beta}}_{\lambda, ridge} = \arg \min_{\beta \in \mathbb{R}^{p+1}} \left( SSR \left(\boldsymbol{\beta} \right) + \lambda  \left\| \beta_{-1} \right\|_{2}^{2} \right), \quad \lambda \geq 0$$

* [**Regresión LASSO**]{.hl-yellow}: cuando $\alpha = 1$ (penalización absoluta)


$$\widehat{\boldsymbol{\beta}}_{\lambda, LASSO} = \arg \min_{\beta \in \mathbb{R}^{p+1}} \left( SSR \left(\boldsymbol{\beta} \right) + \lambda  \left\| \beta_{-1} \right\|_1  \right), \quad \lambda \geq 0$$
...
$$\widehat{\boldsymbol{\beta}}_{s_\lambda, \alpha} = \arg \min_{\beta \in \mathbb{R}^{p+1}:~\sum_{j=1}^{p} \left(\alpha \left| \beta_{j} \right| + (1-\alpha) \left| \beta_{j} \right|^{2} \right) \leq s_\lambda} SSR \left(\boldsymbol{\beta} \right) , \quad 0 \leq \alpha \leq 1$$
...
Las elipses nos muestran los valores para los que $SSR \left( \boldsymbol{\beta} \right)$ es constante (lo que queremos minimizar), y las **regiones sombreadas representan el espacio de valores posibles** debido a la penalización (ver más en <https://link.springer.com/book/10.1007/978-1-4614-7138-7>)
...
$$\widehat{\lambda}_{k-CV} = \arg \min_{\lambda \geq 0} CV_k(\lambda),\quad CV_k(\lambda)=\sum_{j=1}^k  SSR \left(\widehat{\boldsymbol{\beta}_{(j)} } \right)$$
donde $SSR \left(\widehat{\boldsymbol{\beta}}_{(k)} \right)$ es la suma de residuos al cuadrado del $j$-ésimo slot de validación (con los parámetros obtenidos en el entrenamiento sin ese slot).
...
Fíjate que a [**mayor penalización (modelos más simples, más sparsity)**]{.hl-yellow}, el $R^2$ disminuye (o lo que es lo mismo, aumenta el SSR)
...
* `type.measure`: métrica de error usada (por defecto lo que hemos llamado $SSR$ o $MSE$)
...
Fíjate que, de nuevo, a [**mayor penalización (modelos más simples, más sparsity)**]{.hl-yellow}, el $R^2$ disminuye (o lo que es lo mismo, aumenta el SSR)
